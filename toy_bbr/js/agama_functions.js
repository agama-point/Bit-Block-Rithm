...
//(letter above equals below, and vice versa) </pre>
// --- caesar ---
function cipherRot(str,rot) {
  str = str.toUpperCase();
  return str.replace(/[A-Z]/g, rotN);

  function rotN(correspondance) {
    const charCode = correspondance.charCodeAt();
    //A = 65, Z = 90
    return String.fromCharCode(((charCode + rot) <= 90) ? charCode + rot : (charCode + rot) % 90 + 64);    
  }
}
...
// Funkce addSpacesEveryElevenBits z index.js
function addSpacesEveryElevenBits(binaryStr) {
    return binaryStr.match(/.{1,11}/g).join(" ");
}


// Funkce pro převod hexadecimálního řetězce na binární řetězec
function hexToBinary(hex) {
    return hex.match(/.{2}/g).map(byte => ('00000000' + parseInt(byte, 16).toString(2)).slice(-8)).join('');
}

// Funkce pro převod binárního řetězce na desetinný
function binaryToDecimal(binaryStr) {
    return parseInt(binaryStr, 2);
}


// Funkce pro převod hexadecimálního řetězce na ArrayBuffer
...
// Funkce pro převod ArrayBuffer na hexadecimální řetězec
...
// Asynchronní funkce pro SHA-256
async function sha256hex(hexInput) {
    const buffer = hexToArrayBuffer(hexInput); // Převod hexadecimálního řetězce na ArrayBuffer
    const hashBuffer = await crypto.subtle.digest('SHA-256', buffer); // Výpočet SHA-256
    return arrayBufferToHex(hashBuffer); // Převod ArrayBuffer na hexadecimální řetězec
}
...
class dde32 {
   ...
   static I  = "c74a6f342f98f3abcdef124565043dd3";
